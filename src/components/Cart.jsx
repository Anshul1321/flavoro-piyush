...
const Cart = () => {
    ...
    return (
        ...
        <div className={`fixed right-0 top-0 w-full lg:w-[20vw] h-full p-5 bg-white mb-3 ${activeCart ? "translate-x-0" : "translate-x-full"} transition-all duration-500 z-50`}>
            ...
            <span className="text-xl font-boldtext-gray-800">My Order</span>
            <IoMdClose
                onClick={() => setActiveCart(!activeCart)}
                className="border-2 border-gray-600 text-gray-600 font-bold p-1 text-xl rounded-md hover:text-red-300 hover:border-red-300 cursor-pointer"/>
            </div>

             {cartItems. length > 0 ? cartItems.map((food) => {            // Logic to map through the cart items and display them , as well as condition to check if cart is empty or not
                return (
                    <ItemCard 
                    key={food.id}
                    id={food.id}
